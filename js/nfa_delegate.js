var nfa_delegate = (function() {
  ...
  var updateUIForDebug = function() {
    ...
    $('.connection-active').removeClass('connection-active').addClass('connection-normal');
    ...
    $.each(status.states, function(index, state) {
      ...
      jsPlumb.select({source:state}).each(function(connection) {
        if (connection.getLabel() === comparisonChar) {
          $(connection.canvas).removeClass('connection-normal').addClass('connection-active');
        }
      });
    });
    return self;
  ...
  var stepSimulation = function() {
    ...
    updateUIForDebug();
    if (status === 'Active') {
      setTimeout(stepSimulation, 1000); // Adjust the delay as needed
    } else {
      alert('Simulation complete: ' + status);
    }
  };
  var startSimulation = function(input) {
    nfa.stepInit(input);
    stepSimulation();
  };
