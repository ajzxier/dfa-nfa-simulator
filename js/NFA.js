...
NFA.runTests = function() {
  ...
  // Define NFA End AB
  var myNFA = new NFA(true)
    .addTransition('start', 'A', 'start')
    .addTransition('start', 'B', 'start')
    .addTransition('start', 'A', 's1')
    .addTransition('s1', 'B', 's2')
    .addAcceptState('s2');
  ...
  assert(myNFA.accepts('AB'), 'Accept AB');
  assert(myNFA.accepts('AAB'), 'Accept AAB');
  assert(myNFA.accepts('BAAB'), 'Accept BAAB');
  assert(myNFA.accepts('ABAB'), 'Accept ABAB');
  assert(myNFA.accepts('BAAB'), 'Accept BAAB');

  // Reject tests
  assert(!myNFA.accepts(''), 'Reject [emptyString]');
  assert(!myNFA.accepts('A'), 'Reject A');
  assert(!myNFA.accepts('B'), 'Reject B');
  assert(!myNFA.accepts('BA'), 'Reject BA');
  assert(!myNFA.accepts('AA'), 'Reject AA');
  assert(!myNFA.accepts('BB'), 'Reject BB');
  assert(!myNFA.accepts('AABA'), 'Reject AABA');
  assert(!myNFA.accepts('ABA'), 'Reject ABA');

  // Additional tests for NFA functionality
  console.log('Remove transition');
  myNFA.removeTransition('start', 'A', 's1');
  assert(!myNFA.accepts('AB'), 'Reject AB after removing transition start -> A -> s1');

  console.log('Change start state');
  myNFA.setStartState('s1');
  assert(myNFA.accepts('B'), 'Accept B with new start state s1');
  assert(!myNFA.accepts('AAB'), 'Reject AAB with new start state s1');

  console.log('Remove accept state');
  myNFA.removeAcceptState('s2');
  assert(!myNFA.accepts('B'), 'Reject B after removing accept state s2');
  
  console.log('Add new accept state');
  myNFA.addAcceptState('start');
  assert(myNFA.accepts(''), 'Accept [empty string] with new accept state start');
  assert(myNFA.accepts('A'), 'Accept A with new accept state start');

  var myNFA_asString = myNFA.saveToString();
  var otherNFA = new NFA().loadFromString(myNFA_asString);
  assert(myNFA_asString === otherNFA.saveToString(), 'Save, Load, Save has no changes');
  assert(otherNFA.accepts('A'), 'Loaded NFA accepts A');
  assert(!otherNFA.accepts('AAB'), 'Loaded NFA rejects AAB');
  assert(otherNFA.accepts(''), 'Loaded NFA accepts [empty string]');
};
